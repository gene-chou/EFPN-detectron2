...
# flow in this version: c1 -> c2' -> c2 -> c3... rather than c1 -> c2' and c1 -> c2 
# between c1 and c2' we do the 7x7 convolution and no pooling 
# between c2' and c2 we do convolution of stride 2 and c2 has no blocks 



__all__ = [
    "ResNetBlockBase",
    "BasicBlock", # basic residual block for ResNet-18 and ResNet-34; REMOVED
    "BottleneckBlock", #standard bottleneck residual block used by ResNet-50, 101 and 152
    "DeformBottleneckBlock", #deformable; REMOVED
    "BasicStem", # stage 1; 7x7 conv; do not max pool b/t c1 and c2' though 
    "ResNet",
    "make_stage",
...
class BasicStem(CNNBlockBase):
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = F.relu_(x)
        #x = F.max_pool2d(x, kernel_size=3, stride=2, padding=1) --> DO NOT POOL B/T C1 AND C2'
        return x
...
def build_resnet_backbone(cfg, input_shape):
    ...
    for idx, stage_idx in enumerate(range(2, 7)): #max_stage_idx + 1)):
        ...
        stage_kargs = {
            ...
            "in_channels": in_channels, # for original stage 2, in_channels=64, out_channels=256 in default config; which is correct for resnet50,101
